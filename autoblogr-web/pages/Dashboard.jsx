...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // Load stats using real Supabase data
      const ideas = await BlogIdea.findByUserId(user.id);
      // For now, we'll use mock data for posts and sites until they're implemented
      const posts = []; // await BlogPost.findByUserId(user.id);
      const sites = []; // await WordPressSite.findByUserId(user.id);

      setStats({
        totalIdeas: ideas.length,
        totalPosts: posts.length,
        publishedPosts: posts.filter((p) => p.status === "published").length,
        connectedSites: sites.filter((s) => s.connection_status === "connected")
          .length,
      });
        ...
        <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-purple-50 to-purple-100">
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl lg:text-3xl font-bold text-purple-700">
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">
                      {post.word_count} words • Variation{" "}
